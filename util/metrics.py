...
def open_set_classification_metric(true_y, pred_y, is_openset=True, acc=None, label_text_lookup=None, save_path_heatmap=None):
    """It is assumed here that the last class label indicates unknown_class."""
    labels = label_text_lookup.keys()
    cm_labels = label_text_lookup.values()
    cm = metrics.confusion_matrix(true_y, pred_y, labels)
    print(cm)
    plt.figure()
    ax = plt.subplot()
    sns.heatmap(cm, annot=True, fmt='g', ax=ax, vmin=0, vmax=2700);  # annot=True to annotate cells, ftm='g' to disable scientific notation
    ax.set_xlabel('Predicted labels')
    ax.set_ylabel('True labels')
    ax.set_title('Confusion Matrix')
 #   ax.xaxis.set_ticklabels(cm_labels, rotation = 45)
  #  ax.yaxis.set_ticklabels(cm_labels, rotation = 45)
    ...
    if is_openset:
      #  print p, r, f
      #  print 'Known:\nPre={0:4.2}\tRec={1:4.2}\tF-score={2:4.2}'.format(p, r, f)
