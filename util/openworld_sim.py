...
class OpenWorldSim(object):
    ...
    """
    def __init__(self, train_data, train_lable, train_rot=None,
                 val_data=None, val_label=None,
                 test_data=None, test_label=None, test_rot=None,
                 tr_classes=6, validation_classes=[], seed=None):
        np.random.seed(seed)
        if len(train_lable.shape) == 1:
            classes = np.unique(train_lable)
            if not (train_rot is None):
                rots = np.unique(train_rot)
        else:
            classes = np.arange(train_lable.shape[1])
            if not (train_rot is None):
                rots = np.arange(train_rot.shape[1])
    ...
    def train_rot(self):
        if len(self.tr_rot.shape) == 1:
            rot = self.tr_rot[self.tr_mask]
        else:
            rot = self.tr_rot[self.tr_mask]
        return rot
...
class OpenWorldMsData(object):
    """ Create an openworld scenario using the given dataset.
    """
    def __init__(self, data, label, train_rot=None,
                 tr_classes=6, val_classes=[], comb_val_test=False, seed=None,
                 normalize=False):
        """
        ...
        if isinstance(tr_classes, list):
            self.tr_classes =  tr_classes
            self.unknown_classes = [c for c in classes if c not in self.tr_classes]
           # self.val_tr_classes = self.tr_classes.extend(self.unknown_classes[:2])
        else:
            np.random.shuffle(classes)
       ...
       #     self.val_tr_classes = self.tr_classes.extend(self.unknown_classes[:2])
        ...
        if not (train_rot is None):
            tr_x, tr_y, self.tr_rot, val_x, val_y, ts_x, ts_y, tr_classes, val_classes, ts_classes = \
                separate_dataset(data, label, list(self.tr_classes), list(self.val_classes), list(self.unknown_classes),
                                 tvt_ratio=[0.75, 0.0, 0.25], vt_ratio=[0.0, 1.0], seed=seed, rot=train_rot)

        else:
            tr_x, tr_y, val_x, val_y, ts_x, ts_y, tr_classes, val_classes, ts_classes = \
                separate_dataset(data, label, list(self.tr_classes), list(self.val_classes), list(self.unknown_classes),
...
def separate_dataset(xs, ys, num_tr_classes, num_val_classes, num_ts_classes,
                     tvt_ratio=[0.5, 0.25, 0.25], vt_ratio=[0.5, 0.5],  seed=None, rot=None):
    ...
    if not (rot is None):
        tr_r = np.zeros((num_tr, ) + rot.shape[1:])
    ...
    if not (rot is None):
        return tr_x, tr_y, tr_r, val_x, val_y, ts_x, ts_y, tr_classes, val_classes, ts_classes
    else:
